...
#### Using the PeriodGetAutocompleteV1WithHttpInfo variant
...
{
    // Retrieve Periods and IDs
    ApiResponse<CommonGetAutocompleteV1Response> response = apiInstance.PeriodGetAutocompleteV1WithHttpInfo(sSelector, eFilterActive, sQuery, acceptLanguage);
...
{
    Debug.Print("Exception when calling ObjectPeriodApi.PeriodGetAutocompleteV1WithHttpInfo: " + e.Message);
    Debug.Print("Status Code: " + e.ErrorCode);
    Debug.Print(e.StackTrace);
}
```
